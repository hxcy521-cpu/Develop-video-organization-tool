...
> 建议在 **Windows 本机** 执行（不要在 Linux/macOS 里打 Windows EXE）。

### 方式 A：一键脚本（推荐）

在 `video_organizer` 目录双击或命令行执行：
...
```bat
pip install -r requirements.txt
python -m PyInstaller --noconfirm --clean --onefile --windowed --name video_organizer --paths . --collect-submodules app main.py
```
